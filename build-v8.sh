...
gn gen out.gn/$2 --args="$GN_ARGS" && ninja -C out.gn/$2 d8 && pushd . && \
    cd out.gn/$2/obj/ && \
    $NDK_HOME/toolchains/$4/prebuilt/linux-x86_64/bin/$5-ar rcvs libv8.a v8_base/*.o v8_libbase/*.o v8_libsampler/*.o v8_snapshot/*.o v8_libplatform/*.o v8_libsampler/*.o v8_init/*.o v8_initializers/*.o && \
    $NDK_HOME/toolchains/$4/prebuilt/linux-x86_64/bin/$5-strip --strip-debug libv8.a && \
    mkdir /output/$2 && \
    cp libv8.a /output/$2 && \
    popd && rm -r out.gn/$2    
